    ...
    public class WeatherDetailViewModel : BaseViewModel
    {
        ...
        public void SetWeather(string cityName, Weather weather)
        {
            Task.Run(async () =>
            {
                ...
                {
                    await _alertService.DisplayAsync("MCWeather", ex.Message, "Done");
                }
            });
        }
            ...
            try
            {
                ...
                switch (weatherRequest)
                {
                    case LocationWeatherRequest lwr:
                        city = await _locationService.GetCityNameFromLocationAsync(new LocationInfo(lwr.Position.Latitude, lwr.Position.Longitude));
                        break;
                    case WeatherRequest cwr:
                        city = cwr.City.Name;
                        break;
                    default:
                        break;
